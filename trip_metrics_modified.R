# Created by jvorwald and posted to the kaggle forums
# https://www.kaggle.com/c/axa-driver-telematics-analysis/forums/t/12299/any-suggestions-on-doing-local-evaluations/63084#post63084
...
tripFeatures<-function(trip,target,nlag) {
  ...
  #names(tr) <- 'tripId'
  rtn <- c(sd,at,an,atotal,cd,distance)
  return(rtn)
}
...
drivers = list.files("/home/jonathan/dev/kaggle-drivers/drivers/")
for(driver in drivers)
{
  print(driver)
  dirPath = paste0("/home/jonathan/dev/kaggle-drivers/drivers/", driver, '/')
  for(i in 1:200)
  {
    trip = read.csv(paste0(dirPath, i, ".csv"))
    features = tripFeatures(trip, 1, 1)
    write.table(features, paste0("/home/jonathan/dev/kaggle-drivers/features/", driver, "-", i, ".csv"))
  }
}
